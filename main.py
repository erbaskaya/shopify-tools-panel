...
<div class="alert ok"><b>Smart Collection modu:</b> Panel kategori üyeliğine müdahale etmez. İndirim yalnızca <b>varyant stoğu 10'dan büyükse</b> uygulanır. Mevcut satış fiyatı Compare-at Price alanına taşınır; geri almada Compare-at tekrar satış fiyatı olur ve alan temizlenir.</div>
{load_alert}
<style>
...
def job_page(job_id: str, request: Request):
    ...
    label = job.get('progress_label') or 'İşlem'
    pct = min(100, round((current / total) * 100)) if total else 0
    progress = ''
    if total:
        progress = f'<div class="job-progress"><div class="job-progress-text">{e(label)}</div><div class="job-progress-bar"><div class="job-progress-fill" style="width:{pct}%"></div></div><div class="job-progress-count">{current} / {total} ürün - %{pct}</div></div>'
    body = f'<div class="card"><h1>{e(job["title"])}</h1><p><span class="pill status-{e(job["status"])}">{e(job["status"])}</span><span class="muted small">Başlangıç: {e(job["created_at"])}</span></p>{progress}{dl}<pre id="jobLogs">{e(logs)}</pre><a class="btn btn-light" href="/dashboard">Panele dön</a></div><script>(function(){{const el=document.getElementById("jobLogs");if(el)el.scrollTop=el.scrollHeight;}})();</script>'
